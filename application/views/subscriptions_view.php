...
<body onload="initialize()">
						...
						<div class="post">
							<div class="entry">
								<h2>Formulario de registro al evento.</h2>
								    <?php if ($registration_status != "user_registered" && $registration_status != "user_already_registered"): ?>
								<p>Te pedimos que te registres para confirmar tu asistencia, eso nos ayuda a darnos una idea de cuanta gente esperar y tambi&eacute;n es una forma de dar a conocer lo que vas a hacer. Puedes hacerlo con tu cuenta de Twitter <strike>o de Facebook.</strike></p>
									<?php endif;?>
								<?php if ($registration_status == "init" || !empty($registration_verified_user)): ?>						
									<div class="login-buttons">
										<div class="twitter-button">
											<?php if ( $verified_is_registered || (!empty($registration_verified_user) && $registration_verified_user['social_web'] == "twitter") ): ?>
												<img src="<?=base_url()?>assets/images/twitter_signin_inactive.png" alt="Registrate con Twitter" title="Registrate con Twitter">
											<?php else: ?>
												<a href="<?=base_url()?>pages/redirection"><img src="<?=base_url()?>assets/images/twitter_signin.png" alt="Registrate con Twitter" title="Registrate con Twitter"></a>
											<?php endif ?>
										</div>
										<div class="facebook-button">
											<?php if ( $verified_is_registered ): ?>
												<img src="<?=base_url()?>assets/images/facebook_signin_inactive.png" alt="Registrate con Facebook" title="Registrate con Facebook">
											<?php elseif (!empty($me) && (!empty($registration_verified_user) && $registration_verified_user['social_web'] == "twitter")): ?>
												<a href="/"><img src="<?=base_url()?>assets/images/facebook_signin.png" alt="Registrate con Facebook" title="Registrate con Facebook"></a>
											<?php elseif (!empty($me)): ?>
												<img src="<?=base_url()?>assets/images/facebook_signin_inactive.png" alt="Registrate con Facebook" title="Registrate con Facebook">
											<?php else: ?>
								...
								<?php endif ?> 
								<?php if ( !$verified_is_registered && ($registration_status == "twitter_verified" || $registration_status == "facebook_form" || 
																		$registration_status == "form_error") ): ?>
									
									<div class="register_container">
										<div class="register_left">
											<img src="<?php echo $registration_verified_user['social_avatar_url'];?>" width="48" height="48" alt="avatar de usuario"/><br />
											<img src="<?=base_url()?>assets/images/<?php echo $registration_verified_user['social_web']; ?>16x16.png" width="16" height="16" style="margin-top:10px;" alt="social web"/>
										</div>
								
										<div class="register_right">
											<form action="<?=base_url()?>pages/subscriptions" method="post" accept-charset="utf-8" style:"margin: 0px;">
												<input type="hidden" name="form_key" value="<?php echo md5($salt.$verified_user['social_id']); ?>">
												<input type="hidden" name="social_web" value="<?php echo $verified_user['social_web']; ?>">
												<input type="hidden" name="social_id" value="<?php echo $verified_user['social_id']; ?>">
												<input type="hidden" name="social_username" value="<?php echo $verified_user['social_username']; ?>">
												<input type="hidden" name="social_url" value="<?php echo $verified_user['social_url']; ?>">
												<input type="hidden" name="social_avatar_url" value="<?php echo $verified_user['social_avatar_url']; ?>">
												...
												<tbody>
													...
													<tr>
														<td>
															...
															<input type="text" size="20" name="name" value="<?php echo $verified_user['name']; ?>" id="name"><br /> 
														</td>
														<td>
		...
		window.fbAsyncInit = function() {FB.init({appId   : '<?=$f_api?>',session : <?=json_encode($session)?>,status  : true,cookie  : true,xfbml   : true});FB.Event.subscribe('auth.login', function() {window.location.reload();});};(function() {var e = document.createElement('script');e.src = document.location.protocol + '//connect.facebook.net/en_US/all.js';e.async = true;document.getElementById('fb-root').appendChild(e);}());
		</script>
		<script type="text/javascript">
		function initialize() {if (GBrowserIsCompatible()) {var map = new GMap2(document.getElementById("map_canvas"));map.setCenter(new GLatLng(20.677790, -103.372786), 15);var point = new GLatLng(20.677790, -103.372786);map.addOverlay(new GMarker(point));map.setUIToDefault();}}
		</script>
	</body>
</html>
<!--
...
<h3>Lista de Registrados (<?php echo count($users); ?>)</h3>
